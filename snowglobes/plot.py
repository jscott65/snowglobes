...
def plotflux(fluxname, td=False, output_name=False, interactive=False):
    ...
    if td:
        ...
        for i, file in enumerate(files):
            ...
            df[['nue', 'numu', 'nutau', 'nuebar', 'numubar', 'nutaubar']] = df[['nue', 'numu', 'nutau', 'nuebar',
                                                                                'numubar', 'nutaubar']] + df_step[['nue', 'numu', 'nutau', 'nuebar', 'numubar', 'nutaubar']]
    ...
    # logic to determine what to label the lines, and how many there should be
    # based off of which flavs are the same, nux only or both nux and nuxbar.
    # for oscillated fluxes, occasioanlly nue=nuebar thus nu = nue = nuebar
    ...
    ax.set(xlabel='Neutrino Energy (MeV)',
           ylabel=r'Fluence (neutrinos per 0.2 MeV per $cm^2$)',
